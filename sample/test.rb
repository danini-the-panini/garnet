...
test_ok(`#{RUBY_EXE} -e 'print "foobar"'` == 'foobar')

tmp = open("script_tmp", "w")
tmp.print "print $zzz\n";
tmp.close

test_ok(`#{RUBY_EXE} -s script_tmp -zzz` == 'true')
test_ok(`#{RUBY_EXE} -s script_tmp -zzz=555` == '555')
...
test_ok(`#{RUBY_EXE} script_tmp -zzz=678` == '678')
...
test_ok(`#{RUBY_EXE} -x script_tmp` == '')
test_ok(`#{RUBY_EXE} -x script_tmp -zzz=555` == '555')
...
`#{RUBY_EXE} -i.bak -pe '$_.sub!(/^[0-9]+$/){$&.to_i * 5}' script_tmp`
done = true
